import { useEffect, useState } from "react";
import { useNavigate, useParams } from "react-router-dom";
import axios from "axios";

interface Cliente {
  id: number;
  numeroIdentificacion: string;
  razonSocial: string;
  descripcion: string;
  tipoImpuesto: number;
}

export default function ClienteForm() {
  const { id } = useParams();
  const navigate = useNavigate();
  const [cliente, setCliente] = useState<Cliente | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchCliente = async () => {
      try {
        const accessToken = localStorage.getItem("accessToken");
        if (!accessToken) {
          setError("No hay token de acceso disponible.");
          return;
        }

        const response = await axios.post(
          "http://localhost:6061/api/v1/consultar-cliente-id",
          { id: Number(id) },
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
              "Content-Type": "application/json",
            },
          }
        );

        if (response.data.respuesta.esExitosa) {
          setCliente(response.data.resultado);
        } else {
          throw new Error("Error al obtener el cliente.");
        }
      } catch (err: unknown) {
        if (axios.isAxiosError(err)) {
          setError(err.response?.data?.respuesta?.mensaje || "Error en la autenticación");
        } else {
          setError("Error desconocido al obtener el cliente.");
        }
      } finally {
        setLoading(false);
      }
    };

    if (id) {
      fetchCliente();
    }
  }, [id]);

  const handleSubmit = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!cliente || !cliente.id) {
      setError("El cliente no tiene un ID válido.");
      return;
    }
  
    try {
      const accessToken = localStorage.getItem("accessToken");
      if (!accessToken) {
        setError("No hay token de acceso disponible.");
        return;
      }
  
      const response = await axios.post(
        "http://localhost:6061/api/v1/actualizar-cliente",
        {
          id: cliente.id,
          numeroIdentificacion: cliente.numeroIdentificacion,
          razonSocial: cliente.razonSocial,
          descripcion: cliente.descripcion,
          tipoImpuesto: cliente.tipoImpuesto,
        },
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
            "Content-Type": "application/json",
          },
        }
      );
  
      // Validar la estructura del response basado en lo que envía el backend
      if (response.data?.esExitosa) {
        navigate("/cliente");
      } else {
        throw new Error(response.data?.mensaje || "Error al actualizar el cliente.");
      }
    } catch (err: unknown) {
      if (axios.isAxiosError(err)) {
        setError(err.response?.data?.mensaje || "Error en la actualización.");
      } else {
        setError("Error desconocido al actualizar el cliente.");
      }
    }
  };
  
  return (
    <div className="flex flex-col flex-1">
      <div className="flex flex-col justify-center flex-1 w-full max-w-md mx-auto">
        <h1 className="mb-2 font-semibold text-gray-800 text-title-sm dark:text-white/90 sm:text-title-md">
          Editar Cliente
        </h1>
        <form onSubmit={handleSubmit}>
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium">Número de Identificación <span className="text-error-500">*</span></label>
              <input
                type="text"
                className="border p-2 w-full rounded-md"
                placeholder="Número de Identificación"
                value={cliente?.numeroIdentificacion || ""}
                onChange={(e) => setCliente({ ...cliente!, numeroIdentificacion: e.target.value })}
              />
            </div>

            <div>
              <label className="block text-sm font-medium">Razón Social <span className="text-error-500">*</span></label>
              <input
                type="text"
                className="border p-2 w-full rounded-md"
                placeholder="Razón Social"
                value={cliente?.razonSocial || ""}
                onChange={(e) => setCliente({ ...cliente!, razonSocial: e.target.value })}
              />
            </div>

            <div>
              <label className="block text-sm font-medium">Descripción <span className="text-error-500">*</span></label>
              <textarea
                className="border p-2 w-full rounded-md"
                placeholder="Descripción del Cliente"
                value={cliente?.descripcion || ""}
                onChange={(e) => setCliente({ ...cliente!, descripcion: e.target.value })}
              />
            </div>

            <div>
              <label className="block text-sm font-medium">Tipo de Impuesto <span className="text-error-500">*</span></label>
              <select
                className="border p-2 w-full rounded-md"
                value={cliente?.tipoImpuesto || 0}
                onChange={(e) => setCliente({ ...cliente!, tipoImpuesto: Number(e.target.value) })}
              >
                <option value={0}>0</option>
                <option value={1}>1</option>
              </select>
            </div>

            {error && <p className="text-red-500 text-sm">{error}</p>}

            <div>
              <button 
                type="submit" 
                className="w-full bg-blue-500 text-white px-4 py-2 rounded-md"
                disabled={loading}
              >
                {loading ? "Guardando..." : "Guardar Cambios"}
              </button>
            </div>
          </div>
        </form>

        <div className="mt-5 text-center">
          <button 
            onClick={() => navigate("/cliente")} 
            className="text-sm text-gray-700 dark:text-gray-400 hover:text-brand-500"
          >
            Cancelar
          </button>
        </div>
      </div>
    </div>
  );
}
